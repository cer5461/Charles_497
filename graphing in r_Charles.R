...
# term must occur 10 times to be included in the dfm 
...
#it would be good to do this for assholes and not the assholes for the final project
...
#find whether words are being used in the intro, body, or conclusion, etc. 
#who says what at what rates
#project: at what rates are assholes/not the assholes saying words at different rates
textplot_xray(
...
#they talk about whales more at the beginning of the book because Ishmael 
#is a lonesome whaler at the beginning and doesn't join Ahab until the end 
#the end is all about Ahab 
#you can get a glimpse of the narrative arc of certain texts 
textplot_xray(
...
# aes() is one of the main three components of a ggplot command 
# ggplot has a ton of stuff that you can do
# there is a book describing what ggplot does 
# you can change shapes, colors, etc. but that means that it is not super transparent 
# it's so flexible so you have to be googling how it works 
...
# names gives us the character string of names 
# frequency unnames and gives us the numerical string of the names 

# Sort by reverse frequency order
topDf$term <- with(topDf, reorder(term, -frequency))

# this is the same data as the wordcloud, but it is up to you to determine which approach 
# is more valuable 
# for something like this, you absolutely have to get rid of stopwords 
...
# this shows how often these documents use the term american
...
#THINK ABOUT HOW LONG THE DOCUMENTS ARE 
...
# we have to specify the aesthetics of interest in aes()
...
# this graph shows us that different types of cars are doing different things 
ggplot(mpg, aes(x = displ, y = hwy)) +
...
data_char_mobydick
moby_corpus <- corpus(data_char_mobydick)

moby_Dfm <- dfm(moby_corpus, remove =  stopwords('english') , remove_punct = TRUE)
mobyFeatures <- topfeatures(moby_Dfm, 100)
# Create a data.frame for ggplot
moby_topDf <- data.frame(
  list(
    term = names(mobyFeatures),
    frequency = unname(mobyFeatures)
  )
)
# Sort by reverse frequency order
moby_topDf$rank <- seq(1:100)

ggplot(moby_topDf, aes(x=rank, y=frequency)) + geom_smooth()


#smooth doesn't work at first because x and y are not numerical 
ggplot(moby_topDf) + geom_smooth(aes(x=term, y=frequency)) +
  theme(axis.text.x=element_text(angle=90, hjust=1)) 
